...
void redraw(void);
int drawprocfield(Image* screen, int px, int py, char* fmt, ...);
void loaddir(void);
void loadtheme(void);
void usage(void);
...
drawproc(int py)
{
	...
	line->r = Rect(screen->r.min.x+Scrollwidth, screen->r.min.y+Theight+py,
				screen->r.max.x, screen->r.min.y+Theight+py+Vstep);

	draw(screen, line->r, line, nil, ZP);
}
...
redraw(void)
{
	...
	for(i=scroffset; i<nprocs; i++){
		...
		procx += drawprocfield(screen, procx, procy, "%s", proclist[i].user);
		...
		if(argumentflag)
			procx += drawprocfield(screen, procx, procy, "%s", proclist[i].args);
		else
			procx += drawprocfield(screen, procx, procy, "%s", proclist[i].cmd);
		procy += Vstep;
	}
